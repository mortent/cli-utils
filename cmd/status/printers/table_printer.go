...
type tableContentFunc func(w io.Writer, width int, resource *event.ResourceStatus) (int, error)
	...
	columns = []ColumnDef{
		{
			...
			header: "NAMESPACE",
			width:  10,
			content: func(w io.Writer, availableWidth int, resource *event.ResourceStatus) (int, error) {
...
func NewTreePrinter(collector *collector.ResourceStatusCollector, w io.Writer) *TreePrinter {
	return &TreePrinter{
		collector: collector,
		w:         w,
	}
...
func (t *TreePrinter) PrintUntil(stop <-chan struct{}) <-chan struct{} {
...
func (t *TreePrinter) printSubTable(resources []*event.ResourceStatus, prefix string) int {
	linePrintCount := 0
...
func (t *TreePrinter) moveUp() {
	t.printOrDie("%c[%dA", ESC, 1)
}

func (t *TreePrinter) eraseCurrentLine() {
